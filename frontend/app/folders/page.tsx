...
export default function FoldersPage() {
  ...
  async function remove(id: number) {
    if (confirm("Bạn có chắc chắn muốn xóa thư mục này? Các mã QR trong thư mục sẽ chuyển về trạng thái Chưa phân loại.")) {
  ...
  return (
    <DashboardShell>
      ...
      <p className="mt-1 text-sm text-muted">Tạo thư mục để phân loại và quản lý mã QR theo chiến dịch.</p>
      ...
      <section className="mt-6 rounded-md border border-slate-200 bg-white p-5 shadow-soft">
        <div className="grid gap-3 md:grid-cols-[1fr_1fr_auto]">
          <input
            ...
            placeholder="Tên thư mục (ví dụ: Marketing Q3)"
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-3">
        {folders.map((folder) => (
          <div key={folder.id} className="flex flex-col justify-between rounded-md border border-slate-200 bg-white p-4 shadow-soft">
            <div>
              <p className="font-semibold text-ink">{folder.name}</p>
              <p className="mt-1 text-sm text-muted">{folder.description || "Chưa có mô tả"}</p>
            </div>

            <div className="mt-4 flex items-center justify-between gap-2 pt-2 border-t border-slate-100">
              <Link
                href={`/qrcodes?folder_id=${folder.id}`}
              ...
              <Button tone="danger" onClick={() => remove(folder.id)} title="Xóa thư mục">
      ...
      {folders.length === 0 ? (
        <div className="mt-5">
          <EmptyState title="Chưa có thư mục nào" description="Tạo thư mục giúp sắp xếp mã QR gọn gàng hơn." />
        </div>
      ) : null}
    </DashboardShell>
  );
